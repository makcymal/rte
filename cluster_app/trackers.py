...
class CpuTracker(Tracker):
  ...
  def _get_response(self) -> dict:
    ...
    if "freq" in self.fields:
      ...
      response["freq"] = round(
        cpu_freq.current * 1000
        if cpu_freq.current < 10
        else cpu_freq.current
      )
  ...
  def _get_response_percpu(self) -> list[dict]:
    ...
    if cpu_times_percpu:
      cpu_times_fields = self.fields.intersection(
        cpu_times_percpu[0]._fields
      )
      response = [
        {field: getattr(cpu_times, field) for field in cpu_times_fields}
...
class MemTracker(Tracker):
  ...
  def _fill_specs(self):
    self.specs["mem_total"] = round(
      ps.virtual_memory().total / self.bytes_denom
    )
    self.specs["swp_total"] = round(
      ps.swap_memory().total / self.bytes_denom
    )
  ...
  def track(self) -> dict:
    ...
    swp = ps.swap_memory()
    response = {
      field: self._getattr(field, mem, swp) for field in self.fields
    }
    ...
    self.specs = [
      {
        ...
        "total": round(
          ps.disk_usage(part.mountpoint).total / self.bytes_denom
        ),
      }
